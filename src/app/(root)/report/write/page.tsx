...
const WriteReport = () => {
  ...
  const { content, handleContent, submit, loading } = useCreateReport(searchParams.get("workspace"));
  ...
  return (
    <div className="w-full flex items-center flex-col gap-4 pb-4 px-1">
      <div className="w-full max-w-140 h-200 bg-white relative p-10">
        ...
        <textarea className="w-full h-full outline-none text-black border-none resize-none text-xs xl:text-base" placeholder="이번주엔 무엇을 했나요?" onChange={handleContent} value={content} />
      </div>
      <div className="w-full max-w-140 flex gap-2 justify-end items-center">
        ...
        <button onClick={submit} className="p-2 text-xs bg-primary rounded-lg text-center disabled:bg-gray-300" disabled={loading}>{loading ? "게시 중..." : "작성 완료"}</button>
      </div>
    </div>
  )
}
