  ...
  return (
    <div className="w-full h-screen flex items-center justify-center">
      <div className="flex flex-col items-center justify-center gap-2 bg-container border border-border p-4 rounded-lg">
        ...
        <p className="text-lg text-semibold text-center break-keep">워크스페이스의 멘토 권한을 박탈합니다</p>
        ...
        <div className="w-full flex items-center gap-2 mt-2">
          ...
          <button onClick={unsetMentorSubmit} className="flex-1 p-2 text-xs bg-container border border-border rounded text-red-500" disabled={loading}>{loading ? "권한 변경 중..." : "확인"}</button>
        </div>
      </div>
    </div>
  )
