...
const FilePreview = ({ url, name }: FilePreviewProps) => {
  ...
  if (IMAGE_EXT.includes(extension || "")) {
    return <img src={url} alt={name} className="max-w-full lg:h-[700px] m-auto outline-none" />;
  }

  if (VIDEO_EXT.includes(extension || "")) {
    return <video controls src={url} className="max-w-full lg:h-[700px] m-auto outline-none" />;
  }
